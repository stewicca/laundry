  ...
  <body class="bg-[#F5F5F5]">
    <div class="relative max-w-md min-h-screen mx-auto bg-white">
      ...
      <header class="flex items-center justify-center h-20 bg-[#D7F7E8]">
        <h1 class="text-[#13975C] text-2xl font-bold">Edit User</h1>
      </header>

      <main class="max-w-[80%] mx-auto">
        <form action="actions/editUserAction.php" method="POST" class="flex flex-col gap-6 mt-6">
          <input type="number" name="id" value="<?=$user['id']?>" class="hidden" />
          <input type="text" name="name" placeholder="Name" value="<?=$user['name']?>" required class="w-full px-4 py-2 border-2 rounded-lg" />
          <input type="text" name="username" placeholder="Username" value="<?=$user['username']?>" required class="w-full px-4 py-2 border-2 rounded-lg" />
